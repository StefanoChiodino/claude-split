...
def fmt_tokens(n: int, *, suffix: str = " tok") -> str:
    """Format token count for display. Use suffix='' for metrics bar."""
    if n >= 1_000_000:
...
def ticket_card(t: dict) -> Text:
    ...
    ticket_id = t["id"]
    tok = fmt_tokens(t.get("tokens_used", 0))
...
def metrics_view(board: dict, log_mtime: float = 0) -> RenderableType:
    ...
    total_tok = sum(t.get("tokens_used", 0) for t in tickets)
    ...
    line1.append("  \u2502  ")
    line1.append(f"Tokens: {fmt_tokens(total_tok, suffix='') or '0'}", style="bold")
